...
const Calendar = () => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      if (isAuthenticated) {
        await Promise.all([fetchUserRole(), fetchEvents(), fetchLabTests()]);
      }
      setLoading(false);
    };
    initializeAuth();
  ...
  const fetchEvents = async () => {
    const {
      data,
      error
    } = await (supabase as any).from('calendar_events').select('*').order('event_date', {
      ascending: true
    });
    if (error) {
      toast({
  ...
  const fetchLabTests = async () => {
    ...
    const {
      data,
      error
    } = await (supabase as any).from('lab_tests').select('*').eq('patient_id', user.id).order('test_date', {
      ascending: true
    });
    if (error) {
      toast({
  ...
  const handleSaveLabTest = async () => {
    ...
    if (editingLabTest) {
      const {
        error
      } = await (supabase as any).from('lab_tests').update({
      ...
      const {
        error
      } = await (supabase as any).from('lab_tests').insert({
        patient_id: user.id,
        test_name: labTestForm.test_name,
        test_date: labTestForm.test_date,
        notes: labTestForm.notes,
  ...
  const handleDeleteLabTest = async (id: string) => {
    const {
      error
    } = await (supabase as any).from('lab_tests').delete().eq('id', id);
    if (error) {
      toast({
        title: "Error deleting lab test",
        description: error.message,
        variant: "destructive"
      });
    } else {
  ...
  if (loading) {
    return <div className="min-h-screen bg-background">
        <Header currentLanguage={currentLanguage} onLanguageChange={changeLanguage} showCenterLogo />
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="title" value={eventForm.title} onChange={e => setEventForm({
                  ...eventForm,
                ...
                <div>
                  ...
                  <Textarea id="description" value={eventForm.description} onChange={e => setEventForm({
                  ...eventForm,
                ...
                <div>
                  ...
                  <Input id="event_date" type="datetime-local" value={eventForm.event_date} onChange={e => setEventForm({
                  ...eventForm,
                ...
                <div>
                  ...
                  <Select value={eventForm.event_type} onValueChange={(value: any) => setEventForm({
                  ...eventForm,
                  event_type: value
                })}>
                    <SelectTrigger>
